...
async function bienvenida (client, gpu){
    ...
    const welcome = parse(readFileSync('./JSONS/bienvenida.json')); const isWelcome = welcome.includes(gpu.id)
    if(gpu.action == 'add' && isWelcome){
        ...
        let buff = await welcomer.toBuffer()
        writeFileSync('./media/temp/welcome.jpg', buff, 'binary')
        ...
        client.sendMessage(gpu.id, {image: {url: './media/temp/welcome.jpg'}, caption: text, mentions: gpu.participants})
    }
}
